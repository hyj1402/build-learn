...
1. **운영 배포 검증:** Vercel의 로그인·관리자 CRUD·공개/비공개·sitemap/robots/metadata·모바일 흐름을 재확인하고 MDX 보관 정책을 결정합니다.
2. **선택 기능 실검증:** Resend 환경변수로 메일 알림을, `ANTHROPIC_API_KEY`로 다이제스트 품질·비용을 실제 확인합니다.
3. **콘텐츠 운영 개선:** 실제 Log 하나를 각 카테고리로 작성해 공개 탭 노출을 운영 흐름에서 확인한 뒤, 카테고리 관리 UI와 콘텐츠 미리보기·이탈 경고의 필요 범위를 결정합니다.
4. **Tech Radar 자동화:** Cron/GitHub Actions의 실행 주체, 비밀 키 보관, 실패 알림을 설계합니다.

### 변경 원칙

- 새 DB 변경은 새 마이그레이션으로 추가하며, 이미 적용된 마이그레이션은 수정·삭제하지 않습니다.
- RLS·Auth·Storage 변경은 허용·거부 요청과 Advisor를 모두 확인합니다.
- 기존 관리자 UUID가 든 과거 마이그레이션은 현재 권한을 확인한 뒤 별도 운영 작업으로 정리합니다. 새 마이그레이션에 개인 UUID를 하드코딩하지 않습니다.
