...
- 완료: 관리자 방문 로그 — 중복 제한을 통과한 공개 Log·Project 열람은 `content_view_events`에 익명 브라우저 UUID·유입 URL·시각만 기록합니다. `/admin/visits`에서 방문 추이와 인기 콘텐츠를 보며, 이름·이메일·IP·로그인 ID는 기록하지 않습니다.
