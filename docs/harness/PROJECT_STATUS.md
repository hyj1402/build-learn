...
- 완료: Tech Radar 뉴스 분류·다이제스트 대상 선택 — RSS 글은 `개발·AI 기술`·`제품 소식`·`회사·정책·협업`·`검토 필요`으로 자동 1차 분류되며, 기존 글은 안전하게 검토 필요로 보존 후 자동 재분류 또는 수동 보정 가능. 다이제스트는 기본으로 개발·AI 기술의 새 글만 포함하고 필요 시 다른 분류를 선택
...
- 완료: AI 글쓰기 피드백 안전장치 — 관리자 권한 확인 뒤 Claude tool 응답을 런타임에서 검증하고, 같은 관리자 요청의 30초 간격을 서버·화면에서 함께 안내합니다. 미리보기는 본문 스타일 참고용이며 공개 Markdown/MDX 결과와 일부 차이 날 수 있음을 표시합니다.
- 완료: 공개 AI 코멘트 — 관리자가 저장된 Log 본문으로 Claude 초안을 만들고 검토·수정·저장하면 공개 상세에 표시합니다. 생성은 관리자별 30초 간격으로 서버·화면에서 제한하고, AI 초안은 서버에서 600자 이하로 제한합니다. 직접 수정한 코멘트의 저장 한도는 3,000자입니다.
- 완료: Footer 운영·법적 안내 — Contact·이용약관·개인정보처리방침을 한 정보 그룹으로 제공하고, 운영자 작성 콘텐츠 이용 안내와 사이트명을 포함한 저작권 표기를 표시합니다. 개인 사이트에 맞지 않는 주소·사업자·기관 링크는 추가하지 않았습니다.
- 완료: 중복 조회 제한과 Vercel 방문 통계 연결 — 공개 Log·Project는 같은 브라우저의 같은 글을 24시간 동안 한 번만 조회수에 반영합니다(localStorage가 막힌 환경은 본문을 우선 표시하고 기존처럼 요청). Vercel Analytics 수집 코드를 루트에 연결했으며, 대시보드에서 Analytics를 활성화한 다음 Production 배포에서 실제 통계를 확인해야 합니다.
- 완료: 관리자 방문 로그 — 중복 제한을 통과한 공개 Log·Project 열람은 `content_view_events`에 익명 브라우저 UUID·유입 **도메인**·시각만 기록합니다. `/admin/visits`의 오늘·날짜 필터·시각 표시는 한국 시간(Asia/Seoul) 기준이며, 이름·이메일·IP·로그인 ID는 기록하지 않습니다. 보관 기준은 수집일로부터 2년입니다.
- 아직 없음: 카테고리 관리 UI, Tech Radar 자동 수집/다이제스트 실행, 좋아요
